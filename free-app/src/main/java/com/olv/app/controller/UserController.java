...
public class UserController {
    ...
    public RespBody active(@RequestBody ActiveVo vo) {
        ...
        try {
            ...
            if (activeNoLimitInt > 0 && userPo.getActiveNo() <= 0) {
...
//                respBody.add(RespCodeEnum.ERROR.getCode(), msgUtil.getMsg(AppMessage.USER_INVALID, "用户不存在"));
                respBody.add(RespCodeEnum.ERROR.getCode(), msgUtil.getMsg(AppMessage.INVALID_USER, "无效用户"));
                return respBody;
            }
...
//                respBody.add(RespCodeEnum.ERROR.getCode(), msgUtil.getMsg(AppMessage.LOGINPWD_FAIL, "登录密码错误"));
    ...
    public RespBody findUserInfoByUserId(String userId) {
        RespBody respBody = new RespBody();
        try {
            //检验用户是否登录
            AppUserPo appUserPo = commonService.checkToken();
            ...
            //根据用户id获取用户信息
            AppUserPoVo vo=new AppUserPoVo ();
            AppUserPo tranUserPo = userService.findUserById(userId);
            if (tranUserPo == null) {
                tranUserPo = userService.findUserByAddress(userId);
            }
            if (tranUserPo == null) {
                if (userId.length ()>32){
                    if (!bitcoinService.validAddress(userId)) {
                        respBody.add(RespCodeEnum.ERROR.getCode(),msgUtil.getMsg(AppMessage.COIN_ADDRESS_INVAIL, "输入钱包地址无效"));
                        return respBody;
                    }
                }
                vo.setValidAdress (true);
                respBody.add(RespCodeEnum.SUCCESS.getCode(), "合法的钱包地址",vo);
                return respBody;
            }else {
                tranUserPo.setLoginPwd("******");
                tranUserPo.setPayPwd("******");
                tranUserPo.setPayStal("******");
                tranUserPo.setPwdStal("******");
                BeanUtils.copyProperties (vo,tranUserPo);
                vo.setValidAdress (false);
                respBody.add(RespCodeEnum.SUCCESS.getCode(), "获取用户信息成功", vo);
            }
        } catch (Exception ex) {
            respBody.add(RespCodeEnum.ERROR.getCode(), msgUtil.getMsg(AppMessage.GET_USERINFO_FAIL, "获取用户信息失败"));
        }
        return respBody;
    }
    ...
    public RespBody findUserInfo() {
        ...
        try {
            ...
            if (appUserPo.getReleaseTime() == null) {
                ...
                appUserPo.setReleaseTime(calendar.getTime());
            }
            contactPo.setName(appUserPo.getName());
            BeanUtils.copyProperties(vo, appUserPo);
            BeanUtils.copyProperties(vo, contactPo);
            vo.setIsAllowed(appUserPo.getIsAllowed());
            vo.setReleaseTime(appUserPo.getReleaseTime());
            vo.setImgPath(appUserPo.getImgPath());
            vo.setPayPwd("*");
            vo.setLoginPwd("*");
            vo.setPayStal("*");
            vo.setPwdStal("*");
            vo.setAddress(appUserPo.getAddress());
    ...
    public static void main(String[] args) {
        System.out.println(CryptUtils.hmacSHA1Encrypt("as123456", "1b8e252c1abd4a099ad36e3aec170cc2"));
        System.out.println(CryptUtils.hmacSHA1Encrypt("123456", "a77681e81300431faa97fcc4d1ced131"));
    }
