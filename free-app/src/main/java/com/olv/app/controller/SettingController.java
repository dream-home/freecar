...
import com.olv.common.vo.app.UpdateUserVo;
...
//    @RepeatControllerLog
    public RespBody modfiyPwd(@RequestBody UpdateUserVo vo) {
        ...
        try {
            ...
            String newPayPw = CryptUtils.hmacSHA1Encrypt(vo.getNewPwd(), user.getPayStal());
            if (newPayPw.equals(oldPayPw)) {
                respBody.add(RespCodeEnum.ERROR.getCode(), languageUtil.getMsg(AppMessage.NEW_OLD_ERROR, "新旧支付密码不能一致"));
                return respBody;
            }
            //验证通过，更改支付密码
...
//    @RepeatControllerLog
    public RespBody modfiyLoginPwd(@RequestBody UpdateUserVo vo) {
        ...
        try {
            ...
            String newLoginPw = CryptUtils.hmacSHA1Encrypt(vo.getNewLoginPwd (), user.getPwdStal ());
            if (newLoginPw.equals(user.getLoginPwd ())) {
                respBody.add(RespCodeEnum.ERROR.getCode(), languageUtil.getMsg(AppMessage.NEW_OLD_ERROR_LOGIN_ERROR, "新旧登录密码不能一致"));
                return respBody;
            }
            //验证通过，更改登录密码
            AppUserPo model = new AppUserPo();
...
*/
            ...
            //上传图片
            scpUtil.uploadFile(Base64.getDecoder().decode(img), scpUtil.getRemoteRootDir() +"/"+ scpUtil.getPrefix ()+"/headImg", imgName);
            //已经上传的头像要删除掉
            ...
            //验证通过，更换头像
            AppUserPo model = new AppUserPo();
            model.setImgPath(scpUtil.getPrefix ()+"/headImg/"+ imgName);
            //调用通用的更新方法
            userService.updateById(model, user.getId());
...
//                rsList.add(cardVo);
//            }
            respBody.add(RespCodeEnum.SUCCESS.getCode(), languageUtil.getMsg(AppMessage.QUERY_USER_BANK_CARD_LIST_SUCCESS, "查询用户所有银行卡成功"), list);
        } catch (CommException ex) {
            respBody.add(RespCodeEnum.ERROR.getCode(), ex.getMessage());
...
//    @RepeatControllerLog
    public RespBody addBankCard(HttpServletRequest request, @RequestBody BankCardVo cardVo) {
        ...
        try {
            ...
            AppUserPo user = commonService.checkToken();
            String key=user.getId()+":" +request.getRequestURI();
            Boolean f=redisService.redisLock(key,key,10);
            if (!f){
