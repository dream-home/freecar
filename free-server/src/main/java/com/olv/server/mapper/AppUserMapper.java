...
public interface AppUserMapper extends BaseMapper<AppUserPo> {
     ...
     * @return
     */
    public int updateShadowScoreById(@Param("shadowScore") BigDecimal shadowScore, @Param("id") String id);
     ...
     * @return
     */
    @Select("select * from app_user where uid=#{uid}")
    public AppUserPo findUserByUid(@Param("uid") String uid);

    @Update("update app_user set address=#{address} where id = #{id}")
    int updateAddress(@Param("address") String address, @Param("id") String id);

    @Select("select * from app_user where address=#{address}")
    AppUserPo findUserByAddress(String address);
}
